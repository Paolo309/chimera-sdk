...
SNRT_CLUSTER_L1_ZERO(static uint64_t *ki_buffers[N_W_BUFFERS]);
SNRT_CLUSTER_L1_ZERO(static double *kd_buffers[N_W_BUFFERS]);
SNRT_CLUSTER_L1_ZERO(static double *w_buffers[N_W_BUFFERS]);
SNRT_CLUSTER_L1_ZERO(static double *b2_buffers[N_W_BUFFERS]);
SNRT_CLUSTER_L1_ZERO(static double *a2_buffers[N_T_BUFFERS]);
SNRT_CLUSTER_L1_ZERO(static uint64_t *t_buffers[N_T_BUFFERS]);
...
static inline void vexpf_optimized(double *a, double *b) {
    ...
    // double *b2_buffers[N_W_BUFFERS];
    // double *a2_buffers[N_T_BUFFERS];
    ...
    b2_buffers[1] = ALLOCATE_BUFFER(double, BATCH_SIZE);
    b2_buffers[2] = ALLOCATE_BUFFER(double, BATCH_SIZE);
    b2_buffers[0] = ALLOCATE_BUFFER(double, BATCH_SIZE);
    a2_buffers[0] = ALLOCATE_BUFFER(double, BATCH_SIZE);
    a2_buffers[1] = ALLOCATE_BUFFER(double, BATCH_SIZE);
    ...
    for (int iteration = 0; iteration < n_iterations; iteration++) {
        ...
        if (snrt_is_dm_core()) {
            ...
            if (iteration < n_iterations - 4) {
                // Index buffers
                dma_a_ptr = a2_buffers[dma_a_idx];
            ...
            if (iteration > 3) {
                // Index buffers
                dma_b_ptr = b2_buffers[dma_b_idx];
        ...
        if (snrt_cluster_core_idx() == 0) {
            ...
            if (iteration > 0 && iteration < 3 &&
                ...
                // Index buffers
                fp0_a_ptr = a2_buffers[fp0_a_idx];
            ...
            if (iteration > 2 && iteration >= n_iterations - 3 &&
                ...
                fp1_b_ptr = b2_buffers[fp1_w_idx];
