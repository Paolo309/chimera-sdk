...
// TODO implement mxita clock gating

// #define HWPE_CLK_EN_ADDR (SNITCH_CLUSTER_BASE + SNITCH_CLUSTER_PERIPH_BASE + HWPE_CLK_EN_OFFSET)

// static inline void hwpe_clk_enable() {
//   *(volatile uint64_t*)HWPE_CLK_EN_ADDR = 1;
// }

// static inline void hwpe_clk_disable() {
//   *(volatile uint64_t*)HWPE_CLK_EN_ADDR = 0;
// }
...
SNRT_CLUSTER_L1_ZERO(static void *local_input3_matrix);
SNRT_CLUSTER_L1_ZERO(static void *local_weight3_matrix);
SNRT_CLUSTER_L1_ZERO(static void *local_input3_scale);
SNRT_CLUSTER_L1_ZERO(static void *local_weight3_scale);
SNRT_CLUSTER_L1_ZERO(static void *local_output3_matrix);
...
int32_t mxita_test_3csc(void *args) {
    ...
    if (snrt_is_dm_core()) {
        ...
        local_input3_matrix = mxita_l1_alloc(input_mat_size, MXITA_TCDM_ALIGN);
        local_weight3_matrix = mxita_l1_alloc(weight_mat_size, MXITA_TCDM_ALIGN);
        local_input3_scale = mxita_l1_alloc(input_scale_size, MXITA_TCDM_ALIGN);
        local_weight3_scale = mxita_l1_alloc(weight_scale_size, MXITA_TCDM_ALIGN);
        local_output3_matrix = mxita_l1_alloc(output_mat_size, MXITA_TCDM_ALIGN);
        ...
        snrt_dma_start_1d(local_input3_matrix, input_matrix, input_mat_size);
        snrt_dma_start_1d(local_weight3_matrix, weight_matrix, weight_mat_size);
        snrt_dma_start_1d(local_input3_scale, input_scale, input_scale_size);
        snrt_dma_start_1d(local_weight3_scale, weight_scale, weight_scale_size);
    ...
    if (core_idx == 2) {
        ...
        do {
            status3 = hwpe_acquire_job();
        } while (status3 < 0);

        mxita_cfg(k_size, l_size, lk_size, (unsigned int)local_input3_matrix,
                  (unsigned int)local_weight3_matrix, (unsigned int)local_output3_matrix,
