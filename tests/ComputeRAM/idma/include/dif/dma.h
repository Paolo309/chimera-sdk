...
#define X(NAME, BASE_ADDR) \
    ...
    static inline volatile uint32_t *NAME##_dma_nextid_ptr(void) { \
        return (volatile uint32_t *)DMA_NEXTID_ADDR(BASE_ADDR); \
    } \
    static inline volatile uint32_t *NAME##_dma_done_ptr(void) { \
        return (volatile uint32_t *)DMA_DONE_ADDR(BASE_ADDR); \
    } \
