...
DEFAULT_GVSOC_PATH="/app/chimera/chimera-sdk/install/gvsoc/install/bin/gvsoc"
DEFAULT_CMAKE="cmake"
...
validate_target() {
    local target=$1
    if [[ ! -v TARGET_TESTS[$target] ]]; then
        print_error "Invalid target: $target"
        print_info "Available targets: ${!TARGET_TESTS[*]}"
        exit 1
    fi
}

# Function to list tests for a target
list_tests() {
    local target=$1
...
run_tests() {
    ...
    for test in "${tests[@]}"; do
        ...
        set +e
        if run_test "$test" "$gvsoc_path" "$verbose"; then
            ((passed++))
        else
            ((failed++))
            failed_tests+=("$test")
...
if [[ $RUN_ONLY == "false" ]]; then
    build_project "$TARGET" "$CMAKE_CMD" "$TOOLCHAIN_DIR" "$JOBS" "$VERBOSE"
fi

if [[ $BUILD_ONLY == "false" ]]; then
    run_tests "$TARGET" "$GVSOC_PATH" "$VERBOSE" "${TESTS[@]}"
fi