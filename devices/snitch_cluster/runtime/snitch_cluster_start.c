...
static inline void snrt_init_tls() {
    ...
    if (snrt_is_dm_core()) {
        ...
        for (int i = 1; i < snrt_cluster_core_num(); i++) {
            snrt_dma_start_1d((void *)(tls_ptr + i * tls_offset), (void *)tls_ptr, size);
        }
        ...
        for (int i = 0; i < snrt_cluster_core_num(); i++) {
            snrt_dma_start_1d((void *)(tls_ptr + i * tls_offset), (void *)(snrt_zero_memory_ptr()),
                              size);
        }
        snrt_dma_wait_all();
    }
...
static inline void snrt_init_bss() {
    ...
    if (snrt_cluster_idx() == 0 && snrt_is_dm_core()) {
        ...
        snrt_dma_start_1d_wideptr((uint64_t)(&__bss_start), (uint64_t)(snrt_zero_memory_ptr()),
                                  size);
    }
}
#endif
...
#else
    if (snrt_global_core_idx() == 0) *(snrt_exit_code_destination()) = (exit_code << 1) | 1;
#endif
}
